...
class VideoReader:
    ...
    def read_frames(self, path, num_frames, jitter=0, seed=None):
        ...
        if frame_count <= 0: 
            logging.error(f"Error: No frames found in video {path}")
            return None
    ...
    def _read_frames_at_indices(self, path, capture, frame_idxs):
        try:
            ...
            for frame_idx in range(frame_idxs[0], frame_idxs[-1] + 1):
                ...
                if not ret:
                    if self.verbose:
                        logging.warning(f"Error grabbing frame {frame_idx} from movie {path}")
                    break
                ...
                if frame_idx == frame_idxs[current]:
                    ...
                    if not ret or frame is None:
                        if self.verbose:
                            logging.warning(f"Error retrieving frame {frame_idx} from movie {path}")
                        break
            ...
            if self.verbose:
                logging.warning(f"No frames read from movie {path}")
            return None
    ...
    def process_videos(self, input_dir, filenames, video_idxs):
        ...
        for video_idx in video_idxs:
            ...
            try:
                ...
                logging.error(f"Error reading video {filename}: {str(e)}")
                continue
